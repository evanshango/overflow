...
var typesenseUri = builder.Configuration["services:typesense-svc:typesense:0"];
...
app.MapGet("/api/v1/search", async (string query, ITypesenseClient client) =>
{
    string? tag = null;
...
app.MapGet("/api/v1/search/similar-titles", async (string query, ITypesenseClient client) =>
{
