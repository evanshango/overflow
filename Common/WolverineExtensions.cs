    ...
    )
    {
        var retryPolicy = Policy
            ...
            .WaitAndRetryAsync(
                ...
                (_, time, count) =>
                {
                    Console.WriteLine($"Retry attempt {count} failed. Retrying in {time.Seconds} seconds...");
                }
            );

        await retryPolicy.ExecuteAsync(async () =>
        {
            var endpoint = builder.Configuration.GetConnectionString("rabbitmq-svc")
        ...
        {
            opts.UseRabbitMqUsingNamedConnection("rabbitmq-svc")
                .AutoProvision()
